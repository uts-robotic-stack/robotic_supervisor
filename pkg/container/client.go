...
import (
	...
	"github.com/dkhoanguyen/watchtower/pkg/registry"
...
type Client interface {
	...
	CreateContainer(string, container.Config, container.HostConfig, network.NetworkingConfig) (t.ContainerID, error)
	...
	StreamLogs(t.Container, bool, string) (io.ReadCloser, error)
}
...
func (client dockerClient) GetContainer(containerID t.ContainerID) (t.Container, error) {
	...
	if err != nil {
		log.Warnf("Failed to retrieve container image %v info: %v", containerInfo.Name, err)
...
func (client dockerClient) CreateContainer(name string, config container.Config,
...
func (client dockerClient) StartContainerByID(containerID string) error {
	bg := context.Background()
	err := client.api.ContainerStart(bg, containerID, types.ContainerStartOptions{})
	if err != nil {
		return err
	}
	return nil
...
func (client dockerClient) StreamLogs(c t.Container, follow bool, tail string) (io.ReadCloser, error) {
	out, err := client.api.ContainerLogs(context.Background(), c.ContainerInfo().ID, types.ContainerLogsOptions{
		ShowStdout: true, ShowStderr: true, Follow: follow, Details: true, Timestamps: true, Tail: tail})
	if err != nil {
		return nil, err
	}
	return out, nil
